// =================================================================
// Template HTML nativo per Anteprima Rapporto Tecnico
// =================================================================
// Replica fedele del layout Word (rapporto-template.docx).
// Struttura tabelle da analisi XML del docx (gridCol, gridSpan, trHeight).
// Due pagine A4 esplicite, side-by-side in anteprima.
// =================================================================

const LOGO_B64 = '';

const PAGE_CSS = `
* { margin:0; padding:0; box-sizing:border-box; }
body { font-family: Calibri, 'Segoe UI', Arial, sans-serif; font-size: 9pt; color: #333; background: transparent; }
.page {
  width: 210mm; min-height: 297mm; padding: 12mm 15mm 10mm 15mm;
  background: #fff; position: relative;
}
@media print {
  .page { page-break-after: always; padding: 10mm 15mm; }
  .page:last-child { page-break-after: auto; }
}
:root {
  --hdr: #005B8C;
  --label-bg: #D5E8F0;
  --border: #8DB3D4;
}
.logo-bar { text-align: center; margin-bottom: 4px; }
.logo-bar img { height: 28mm; }
.doc-title { text-align: center; color: var(--hdr); font-size: 10pt; font-weight: bold; margin-bottom: 1px; }
.doc-subtitle { text-align: center; color: var(--hdr); font-size: 7.5pt; font-style: italic; margin-bottom: 6px; }
table { border-collapse: collapse; width: 100%; }
td, th { border: 1px solid var(--border); vertical-align: middle; }

.t1 col.c-chk  { width: 4.8%; }
.t1 col.c-desc { width: 43.9%; }
.t1 col.c-art  { width: 10.3%; }
.t1 col.c-dich { width: 10.3%; }
.t1 col.c-irr  { width: 10.3%; }
.t1 col.c-grav { width: 10.3%; }
.t1 col.c-rec  { width: 10.3%; }
.t1 .banner { background: var(--hdr); color: #fff; text-align: center; font-weight: bold; font-size: 10pt; height: 7mm; }
.t1 .pratica { text-align: center; color: var(--hdr); font-weight: bold; font-size: 9pt; height: 30mm; line-height: 1.5; }
.t1 .sotto-lbl { background: var(--label-bg); color: var(--hdr); font-weight: bold; font-size: 8pt; height: 6mm; padding: 0 4px; }
.t1 .sotto-val { font-size: 8.5pt; height: 6mm; padding: 0 4px; }
.t1 .title-infr { text-align: center; color: var(--hdr); font-weight: bold; font-size: 9pt; height: 13.8mm; }
.t1 .hdr { background: var(--hdr); color: #fff; font-weight: bold; font-size: 7.5pt; text-align: center; height: 7mm; }
.t1 .sub { background: var(--label-bg); color: var(--hdr); font-weight: bold; font-size: 7.5pt; text-align: center; height: 6mm; }
.t1 .art-chk { text-align: center; font-weight: bold; font-size: 8pt; height: 6mm; }
.t1 .art-desc { text-align: left; font-size: 7pt; height: 6mm; padding: 1px 3px; }
.t1 .art-val { text-align: center; font-size: 7.5pt; height: 6mm; }
.t1 tbody tr:nth-child(odd) td { background: #f5f9fc; }
.t1 .rimborso { background: var(--label-bg); color: var(--hdr); font-weight: bold; font-size: 7.5pt; height: 6mm; text-align: left; padding: 1px 6px; }
.t1 .importo { font-size: 8pt; height: 6mm; padding: 1px 6px; }
.t1 .importo-note { font-size: 7pt; font-style: italic; color: #666; padding: 1px 6px; }

.t2 col.p0 { width: 16.3%; }
.t2 col.p1 { width: 12.3%; }
.t2 col.p2 { width: 4.7%; }
.t2 col.p3 { width: 1.3%; }
.t2 col.p4 { width: 15.4%; }
.t2 col.p5 { width: 16.6%; }
.t2 col.p6 { width: 0.3%; }
.t2 col.p7 { width: 12.1%; }
.t2 col.p8 { width: 21.1%; }
.t2 .section-banner { background: var(--hdr); color: #fff; text-align: center; font-weight: bold; font-size: 9pt; height: 10mm; }
.t2 .field-label { background: var(--label-bg); color: var(--hdr); font-weight: bold; font-size: 8pt; height: 6mm; padding: 2px 6px; }
.t2 .fv-desc { height: 35mm; font-size: 8.5pt; padding: 3px 6px; vertical-align: top; white-space: pre-wrap; }
.t2 .fv-circ { height: 20mm; font-size: 8.5pt; padding: 3px 6px; vertical-align: top; white-space: pre-wrap; }
.t2 .fv-luogo { height: 20mm; font-size: 8.5pt; padding: 3px 6px; vertical-align: top; white-space: pre-wrap; }
.t2 .sub-data { color: var(--hdr); font-weight: bold; font-size: 8pt; height: 6mm; padding: 2px 6px; }
.t2 .section-hdr { font-weight: bold; font-size: 8.5pt; color: var(--hdr); height: 6mm; padding: 2px 6px; }
.t2 .lbl { background: var(--label-bg); color: var(--hdr); font-weight: bold; font-size: 8pt; height: 6mm; padding: 2px 6px; white-space: nowrap; }
.t2 .val { font-size: 8.5pt; height: 6mm; padding: 2px 6px; }
.t2 .pres-lbl { color: var(--hdr); font-weight: bold; font-size: 8pt; height: 6mm; padding: 2px 6px; }
.t2 .pres-val { font-size: 8.5pt; height: 6mm; padding: 2px 6px; }
.t2 .firma-lbl { font-weight: bold; color: var(--hdr); font-size: 8pt; height: 6mm; padding: 2px 6px; text-align: center; }
.t2 .firma-val { font-size: 8.5pt; height: 6mm; padding: 2px 6px; }
.t2 .sign-lbl { font-weight: bold; color: var(--hdr); font-size: 8.5pt; height: 6mm; padding: 2px 6px; }
.t2 .sign-val { font-size: 8.5pt; height: 6mm; padding: 2px 6px; }
`;

export const RAPPORTO_WEB_TEMPLATE = `<!DOCTYPE html>
<html lang="it">
<head>
<meta charset="utf-8">
<title>Rapporto Tecnico di Rilevazione</title>
<style>${PAGE_CSS}</style>
</head>
<body>

<!-- PAGINA 1 -->
<div class="page" id="page1">

  <div class="logo-bar"><img src="data:image/jpeg;base64,${LOGO_B64}" alt="CBSM"></div>
  <div class="doc-title">Violazione delle Norme Generali sulla distribuzione dell&rsquo;acqua ad uso irriguo</div>
  <div class="doc-subtitle">(approvate con delibera del Consiglio dei Delegati n. 7 del 28 giugno 2024)</div>

  <table class="t1">
    <colgroup>
      <col class="c-chk"><col class="c-desc"><col class="c-art">
      <col class="c-dich"><col class="c-irr"><col class="c-grav"><col class="c-rec">
    </colgroup>
    <tr><td colspan="7" class="banner">RAPPORTO TECNICO DI RILEVAZIONE</td></tr>
    <tr><td colspan="7" class="pratica">
      N: {{cod_pratica}} &ndash; ANNO: {{anno}}<br>
      AREA {{area_label}}<br>
      SETTORE {{settore_label}}
    </td></tr>
    <tr>
      <td colspan="1" class="sotto-lbl">Il sottoscritto</td>
      <td colspan="1" class="sotto-val">{{tecnico_rilevatore}}</td>
      <td colspan="1" class="sotto-lbl">il giorno</td>
      <td colspan="1" class="sotto-val">{{data_rilevazione}}</td>
      <td colspan="1" class="sotto-lbl">alle ore</td>
      <td colspan="2" class="sotto-val">{{ora_rilevazione}}</td>
    </tr>
    <tr><td colspan="7" class="title-infr">HA RILEVATO LE SEGUENTI INFRAZIONI</td></tr>
    <tr>
      <td colspan="3" class="hdr">Infrazione</td>
      <td colspan="2" class="hdr">Superfici (m&sup2;)</td>
      <td class="hdr">Gravit&agrave;<br><span style="font-weight:normal;font-size:6pt">(da 1 a 4)</span></td>
      <td class="hdr">Recidiva<br><span style="font-weight:normal;font-size:6pt">(x)</span></td>
    </tr>
    <tr>
      <td class="sub">(x)</td>
      <td class="sub">Descrizione</td>
      <td class="sub">Articolo</td>
      <td class="sub">Dichiarata</td>
      <td class="sub">Irrigata</td>
      <td class="sub">(da 1 a 4)</td>
      <td class="sub">(x)</td>
    </tr>
    <tbody>
    <tr><td class="art-chk">{{x_art08}}</td><td class="art-desc">Violazione servizio reperibilit&agrave;</td><td class="art-val">8</td><td class="art-val">{{sup_dich_art08}}</td><td class="art-val">{{sup_irr_art08}}</td><td class="art-val">{{grado_art08}}</td><td class="art-val">{{recidiva_art08}}</td></tr>
    <tr><td class="art-chk">{{x_art12}}</td><td class="art-desc">Negato accesso ai fondi (al personale consortile)</td><td class="art-val">12</td><td class="art-val">{{sup_dich_art12}}</td><td class="art-val">{{sup_irr_art12}}</td><td class="art-val">{{grado_art12}}</td><td class="art-val">{{recidiva_art12}}</td></tr>
    <tr><td class="art-chk">{{x_art15}}</td><td class="art-desc">Prelievo abusivo d&rsquo;acqua</td><td class="art-val">15</td><td class="art-val">{{sup_dich_art15}}</td><td class="art-val">{{sup_irr_art15}}</td><td class="art-val"></td><td class="art-val">{{recidiva_art15}}</td></tr>
    <tr><td class="art-chk">{{x_art16}}</td><td class="art-desc">Inosservanza termini presentazione comunicazioni (comunicazione di irrigazione tardiva)</td><td class="art-val">16</td><td class="art-val">{{sup_dich_art16}}</td><td class="art-val">{{sup_irr_art16}}</td><td class="art-val">{{grado_art16}}</td><td class="art-val">{{recidiva_art16}}</td></tr>
    <tr><td class="art-chk">{{x_art17}}</td><td class="art-desc">Inosservanza termini presentazione comunicazioni (comunicazione di variazione o di rinuncia tardive)</td><td class="art-val">17</td><td class="art-val">{{sup_dich_art17}}</td><td class="art-val">{{sup_irr_art17}}</td><td class="art-val">{{grado_art17}}</td><td class="art-val">{{recidiva_art17}}</td></tr>
    <tr><td class="art-chk">{{x_art27}}</td><td class="art-desc">Spreco d&rsquo;acqua / Uso negligente risorse idriche</td><td class="art-val">27</td><td class="art-val">{{sup_dich_art27}}</td><td class="art-val">{{sup_irr_art27}}</td><td class="art-val">{{grado_art27}}</td><td class="art-val">{{recidiva_art27}}</td></tr>
    <tr><td class="art-chk">{{x_art28}}</td><td class="art-desc">Violazione prescrizioni del Consorzio</td><td class="art-val">28</td><td class="art-val">{{sup_dich_art28}}</td><td class="art-val">{{sup_irr_art28}}</td><td class="art-val">{{grado_art28}}</td><td class="art-val">{{recidiva_art28}}</td></tr>
    <tr><td class="art-chk">{{x_art29}}</td><td class="art-desc">Violazione termini restituzione attrezzature</td><td class="art-val">29</td><td class="art-val">{{sup_dich_art29}}</td><td class="art-val">{{sup_irr_art29}}</td><td class="art-val">{{grado_art29}}</td><td class="art-val">{{recidiva_art29}}</td></tr>
    <tr><td class="art-chk">{{x_art30}}</td><td class="art-desc">Danneggiamento e/o perdita attrezzature</td><td class="art-val">30</td><td class="art-val">{{sup_dich_art30}}</td><td class="art-val">{{sup_irr_art30}}</td><td class="art-val">{{grado_art30}}</td><td class="art-val">{{recidiva_art30}}</td></tr>
    <tr><td class="art-chk">{{x_art31}}</td><td class="art-desc">Mancata segnalazione guasti</td><td class="art-val">31</td><td class="art-val">{{sup_dich_art31}}</td><td class="art-val">{{sup_irr_art31}}</td><td class="art-val">{{grado_art31}}</td><td class="art-val">{{recidiva_art31}}</td></tr>
    <tr><td class="art-chk">{{x_art32}}</td><td class="art-desc">Negato accesso ai fondi (al consorziato)</td><td class="art-val">32</td><td class="art-val">{{sup_dich_art32}}</td><td class="art-val">{{sup_irr_art32}}</td><td class="art-val">{{grado_art32}}</td><td class="art-val">{{recidiva_art32}}</td></tr>
    <tr><td class="art-chk">{{x_art33}}</td><td class="art-desc">Inosservanza limiti temporali di prelievo</td><td class="art-val">33</td><td class="art-val">{{sup_dich_art33}}</td><td class="art-val">{{sup_irr_art33}}</td><td class="art-val">{{grado_art33}}</td><td class="art-val">{{recidiva_art33}}</td></tr>
    <tr><td class="art-chk">{{x_art34}}</td><td class="art-desc">Interferenze</td><td class="art-val">34</td><td class="art-val">{{sup_dich_art34}}</td><td class="art-val">{{sup_irr_art34}}</td><td class="art-val">{{grado_art34}}</td><td class="art-val">{{recidiva_art34}}</td></tr>
    <tr><td class="art-chk">{{x_art35}}</td><td class="art-desc">Manomissione reti di dispensa e allaccio di apparecchi di aspirazione all&rsquo;idrante</td><td class="art-val">35</td><td class="art-val">{{sup_dich_art35}}</td><td class="art-val">{{sup_irr_art35}}</td><td class="art-val">{{grado_art35}}</td><td class="art-val">{{recidiva_art35}}</td></tr>
    <tr><td class="art-chk">{{x_art36}}</td><td class="art-desc">Uso attrezzature non autorizzate</td><td class="art-val">36</td><td class="art-val">{{sup_dich_art36}}</td><td class="art-val">{{sup_irr_art36}}</td><td class="art-val">{{grado_art36}}</td><td class="art-val">{{recidiva_art36}}</td></tr>
    <tr><td class="art-chk">{{x_art37}}</td><td class="art-desc">Uso sistemi di irrigazione incompatibili</td><td class="art-val">37</td><td class="art-val">{{sup_dich_art37}}</td><td class="art-val">{{sup_irr_art37}}</td><td class="art-val">{{grado_art37}}</td><td class="art-val">{{recidiva_art37}}</td></tr>
    <tr><td class="art-chk">{{x_art39}}</td><td class="art-desc">Danni strutture irrigue</td><td class="art-val">39</td><td class="art-val">{{sup_dich_art39}}</td><td class="art-val">{{sup_irr_art39}}</td><td class="art-val">{{grado_art39}}</td><td class="art-val">{{recidiva_art39}}</td></tr>
    </tbody>
    <tr><td colspan="7" class="rimborso">Rimborso spese e/o risarcimento danni <span style="font-weight:normal;font-style:italic;font-size:6.5pt">(articoli n. 8, 27, 30) (*)</span></td></tr>
    <tr>
      <td class="importo" style="font-weight:bold;font-size:7.5pt;">Importo a pi&egrave; di lista (&euro;):</td>
      <td class="importo">{{importo_rimborso}}</td>
      <td colspan="5" class="importo-note">(Vedi nota spesa allegata)</td>
    </tr>
  </table>

</div>

<!-- PAGINA 2 -->
<div class="page" id="page2">

  <table class="t2">
    <colgroup>
      <col class="p0"><col class="p1"><col class="p2"><col class="p3">
      <col class="p4"><col class="p5"><col class="p6"><col class="p7"><col class="p8">
    </colgroup>
    <tr><td colspan="9" class="section-banner">DATI DELLA RILEVAZIONE</td></tr>
    <tr><td colspan="9" class="field-label">Descrizione dettagliata dell&rsquo;infrazione rilevata</td></tr>
    <tr><td colspan="9" class="fv-desc">{{descrizione_fatti}}</td></tr>
    <tr><td colspan="9" class="field-label">Altre circostanze rilevanti</td></tr>
    <tr><td colspan="9" class="fv-circ">{{circostanze}}</td></tr>
    <tr><td colspan="9" class="field-label">Descrizione dei luoghi</td></tr>
    <tr><td colspan="9" class="fv-luogo">{{descrizione_luogo}}</td></tr>
    <tr><td colspan="9" class="field-label">Altri dati</td></tr>
    <tr>
      <td colspan="5" class="sub-data">Distretto irriguo: {{distretto_irriguo}}</td>
      <td colspan="4" class="sub-data">Comizio: {{comizio}}</td>
    </tr>
    <tr>
      <td colspan="3" class="sub-data">Idrante: {{idrante}}</td>
      <td colspan="3" class="sub-data">Matricola contatore: {{matricola_contatore}}</td>
      <td colspan="3" class="sub-data">Matricola tessera: {{matricola_tessera}}</td>
    </tr>
    <tr><td colspan="9" class="section-banner">DATI DEL TRASGRESSORE</td></tr>
    <tr><td colspan="9" class="section-hdr">Dati generali</td></tr>
    <tr>
      <td colspan="1" class="lbl">Denominazione</td>
      <td colspan="6" class="val">{{denominazione}}</td>
      <td colspan="1" class="lbl">{{cf_piva_label}}</td>
      <td colspan="1" class="val">{{cf_piva}}</td>
    </tr>
    <tr><td colspan="9" class="section-hdr">Residenza</td></tr>
    <tr>
      <td colspan="1" class="lbl">Via</td>
      <td colspan="6" class="val">{{via}}</td>
      <td colspan="1" class="lbl">n.</td>
      <td colspan="1" class="val">{{civico}}</td>
    </tr>
    <tr>
      <td colspan="1" class="lbl">Localit&agrave;</td>
      <td colspan="6" class="val">{{localita}}</td>
      <td colspan="1" class="lbl">Comune</td>
      <td colspan="1" class="val">{{citta}}</td>
    </tr>
    <tr>
      <td colspan="1" class="lbl">Telefono</td>
      <td colspan="6" class="val">{{telefono}}</td>
      <td colspan="1" class="lbl">e-mail/PEC</td>
      <td colspan="1" class="val">{{email_pec}}</td>
    </tr>
    <tr>
      <td colspan="2" class="pres-lbl">Il Trasgressore era presente?</td>
      <td colspan="2" class="pres-val">{{presenza_trasgressore}}</td>
      <td colspan="4" class="firma-lbl">Firma del trasgressore</td>
      <td colspan="1" class="firma-val"></td>
    </tr>
    <tr><td colspan="9" class="section-banner">FIRMATO</td></tr>
    <tr>
      <td colspan="2" class="sign-lbl">Il Tecnico Rilevatore:</td>
      <td colspan="7" class="sign-val">{{firma_tr}}</td>
    </tr>
    <tr>
      <td colspan="2" class="sign-lbl">Il Tecnico Istruttore:</td>
      <td colspan="7" class="sign-val">{{firma_ti}}</td>
    </tr>
    <tr>
      <td colspan="2" class="sign-lbl">Il Responsabile del Settore:</td>
      <td colspan="7" class="sign-val">{{firma_rz}}</td>
    </tr>
    <tr>
      <td colspan="2" class="sign-lbl">Il Responsabile Istruttore:</td>
      <td colspan="7" class="sign-val">{{firma_ri}}</td>
    </tr>
    <tr>
      <td colspan="2" class="sign-lbl">Il Direttore dell&rsquo;Area:</td>
      <td colspan="7" class="sign-val">{{firma_dt}}</td>
    </tr>
  </table>

</div>

</body>
</html>`;
