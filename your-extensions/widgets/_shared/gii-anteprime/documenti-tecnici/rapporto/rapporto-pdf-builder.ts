...
export function buildRapportoIterPlaceholders (opts: {
  ...
  const iterItCompilazione = findLastChiusuraFromCicli(cicli, 'IT', ['ISTRUTTORIA_TRASMESSA_VERIFICA', 'ESITO_INTEGRAZIONE_TRASMESSO', 'NUOVA_RILEVAZIONE_TRASMESSA'], ['CS'])
  const iterCsVerifica = findLastChiusuraFromCicli(cicli, 'CS', ['ISTRUTTORIA_VERIFICATA'], ['RIT']) || ''
  const iterRitSupervisione = findLastChiusuraFromCicli(cicli, 'RIT', ['ISTRUTTORIA_TECNICA_VALIDATA', 'ESITO_INTEGRAZIONE_TRASMESSO'], ['DT']) || (!hasIterCicli ? dateFrom(d, 'dt_stato_RIT') : '')
