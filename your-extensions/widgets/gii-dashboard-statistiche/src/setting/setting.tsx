...
export default function Setting (props: AllWidgetSettingProps<IMConfig>) {
  ...
  return (
    <div style={P.wrap}>
      ...
      <ColInp value={cfg.accentColor || '#38bdf8'} onChange={v => update('accentColor', v)} />
      <label style={P.lbl}>Sfondo pannello</label>
      <ColInp value={cfg.panelBg || 'rgba(3,10,24,0.94)'} onChange={v => update('panelBg', v)} />
      <label style={P.lbl}>Sfondo card</label>
      <ColInp value={cfg.cardBg || 'rgba(15,23,42,0.74)'} onChange={v => update('cardBg', v)} />
      <label style={P.lbl}>Bordo card</label>
      <ColInp value={cfg.cardBorder || 'rgba(148,163,184,0.20)'} onChange={v => update('cardBorder', v)} />
      ...
      <ColInp value={cfg.mutedColor || 'rgba(203,213,225,0.72)'} onChange={v => update('mutedColor', v)} />
    </div>
  )
}
