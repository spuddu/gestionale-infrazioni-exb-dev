...
// ✅ type-guard (risolve TS2339 su res.error in modo definitivo)
function isTakeError (r: TakeResult): r is { ok: false; error: string } {
  return r.ok === false
}
...
export default function Widget (props: Props): React.ReactElement {
  // ---- merge defaultConfig + config (con merge profondo di colWidths)
  ...
  const colsCount = 6
  const minWidth =
    wPratica + wData + wUfficio + wStato + wDtPresa + wBtn +
    gap * (colsCount - 1)
  ...
  const styles = css`
    ...
    /* ✅ QUI: nessun padding orizzontale -> niente inset “misterioso” sull’header */
    .content {
    ...
    .headerWrap {
      ...
      /* ✅ azzero tutto quello che può creare offset */
    ...
    /* ✅ IMPORTANTISSIMO: annullo padding laterali imposti dal tema */
    .cell {
      min-width: 0;
      white-space: nowrap;
      overflow: hidden;
    ...
    /* ✅ unica “leva” che sposta la prima colonna (header e righe) */
    .first {
      padding-left: ${padFirst}px !important;
    }
  ...
  }, [cfg.whereClause, cfg.pageSize, cfg.orderByField, cfg.orderByDir])
